...
parser.add_argument("--logdir",type=str,default='logs15/',help="Directory to save encoder")
...
lang_pair = params.lp
hyppath = params.hyppath
domain = os.path.basename(hyppath).split('.')[0]
sys_name = ".".join(os.path.basename(hyppath).split('.')[1:-1])
...
for i in range(0,sz,1):
    ...
    e_avg, e_prod = (e1_score + e2_score)/2, e1_score*e2_score
    ...
    curr_line2 = [str(i+1),lang_pair,domain,sys_name,str(e1_score),str(e2_score),str(e_prod),str(e_avg),str(result),first[i],second[i]]
